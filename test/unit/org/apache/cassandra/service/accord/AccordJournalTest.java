...
public class AccordJournalTest
{
    ...
    public void compare()
    {
        qt().forAll(Gens.lists(keyGen()).ofSizeBetween(2, 100)).check(keys -> {
            Collections.sort(keys, Key.SUPPORT::compare);
            List<ByteBuffer> bbs = new ArrayList<>(keys.size());
            for (Key k : keys)
                bbs.add(toBB(k));

            for (int i = 0; i < keys.size(); i++)
            {
                Key outter = keys.get(i);
                for (int j = 0; j < keys.size(); j++)
                {
                    Key inner = keys.get(j);
                    ByteBuffer bb = bbs.get(j);
                    Order expected = FBUtilities.compare(outter, inner, Key.SUPPORT::compare);
                    Order actual = FBUtilities.compare(outter, bb, new AsymmetricOrdering<Key, ByteBuffer>()
                    {
                        @Override
                        ...
                        @Override
                        public int compare(@Nullable Key key, @Nullable Key t1)
                        {
    ...
    private static ByteBuffer toBB(Key k)
    {
    ...
    private Gen<Key> keyGen()
    {
        ...
        Gen<Key> gen = rs -> new Key(txnIdGen.next(rs), typeGen.next(rs), storeGen.nextInt(rs));
        return gen;
    }
}