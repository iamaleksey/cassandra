...
public class VirtualTableTest extends CQLTester
{
    ...
    public void testNetPaging() throws Throwable
    {
        for (int i = 0; i < 10; i++)
        {
            ...
            netRs = executeNetWithPaging("SELECT * FROM " + keyspace.name + ".vtable LIMIT 3;", 100000);
            assertEquals(netRs.all().size(), 3);
        }
    }
    ...
    {
        compareRepl(MessageFormat.format(q, KEYSPACE + "." + currentTable()),
                MessageFormat.format(q, keyspace.name + ".vtable"));
    }

    private void compareRepl(String real, String virtual) throws Throwable
    {
        int pageSize = new Random().nextInt(100) + 1;
        logger.info("pager size: " + pageSize);
        ResultSet rel = executeNetWithPaging(real, pageSize);
        ResultSet virt = executeNetWithPaging(virtual, pageSize);
        UntypedResultSet internalReal = QueryProcessor.executeInternalWithPaging(real, pageSize);
        UntypedResultSet internalVirtual = QueryProcessor.executeInternalWithPaging(virtual, pageSize);

        Iterator<com.datastax.driver.core.Row> realRows = rel.iterator();
        for (com.datastax.driver.core.Row row : virt)
        {
            com.datastax.driver.core.Row realRow = realRows.next();
            for (Definition d : row.getColumnDefinitions().asList())
            {
                assertEquals(realRow.getBytesUnsafe(d.getName()), row.getBytesUnsafe(d.getName()));
            }
        }

        Iterator<UntypedResultSet.Row> internalRealRows = internalReal.iterator();
        for (UntypedResultSet.Row row : internalVirtual)
        {
            UntypedResultSet.Row r = internalRealRows.next();
            for (ColumnSpecification d : r.getColumns())
            {
                assertEquals(r.getBlob(d.name.toString()), row.getBlob(d.name.toString()));
            }
        }
    }
