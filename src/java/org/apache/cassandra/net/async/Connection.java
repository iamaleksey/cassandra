...
class Connection implements MessageCallbacks, MessageProcessor
{
    ...
    public void process(Message<?> message, int messageSize, MessageCallbacks callbacks)
    {
        ...
        if (!Arrays.equals((byte[])canon.payload, (byte[]) message.payload)) // assertArrayEquals is EXTREMELY inefficient
            throw new AssertionError("payload bytes are not equal");
    ...
    private void onFailed(int messageSize, long id)
    {
        ...
        if (canon.serializedSize(current_version) != messageSize)
            throw new AssertionError("messages sizes are not equal");
...
//        System.out.println("C " + id);
        ...
        if ((long)inFlight.remove(id) != (long)messageSize)
            throw new AssertionError("message sizes don't match");
        ...
        if (oldest.getValue() <= onEventLoopThreshold)
        {
            ...
            if (oldest.getKey() != id)
            {
                synchronized (this)
                {
                    throw new AssertionError(minId + " " + minReceiveId + " " + oldest.getKey() + " " + maxId + " " +  inFlight.toString());
                }
            }
        }
