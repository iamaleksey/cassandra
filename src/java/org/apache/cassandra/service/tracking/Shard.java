...
public class Shard
{
    ...
    Shard(String keyspace, Range<Token> tokenRange, int localHostId, Participants participants, Epoch sinceEpoch, IntSupplier logIdProvider)
    {
        ...
        logs.put(currentLocalLog.id.asLong(), currentLocalLog);
    }
    ...
    public void witnessedMutation(MutationId id, int onHostId)
    {
        get(id).witnessedMutation(id, onHostId);
    }

    public void witnessedMutations(long logId, SequenceIds ranges, int onHostId)
    {
        get(logId).witnessedMutations(ranges, onHostId);
    }
    ...
    private static CoordinatorLog.CoordinatorLogPrimary startNewLog(int localHostId, int hostLogId, Participants participants)
    {
        ...
        return new CoordinatorLog.CoordinatorLogPrimary(logId, participants);
    }
    ...
    private CoordinatorLog get(long logId)
    {
        ...
        if (log != null)
            return log;
        return logs.computeIfAbsent(logId, ignore -> CoordinatorLog.create(localHostId, new CoordinatorLogId(logId), participants));
    }
}
