...
public class PagingState
{
    ...
    /**
     * It's possible to receive a V3 paging state on a V4 client session, and vise versa - so we cannot
    ...
    private ByteBuffer modernSerialize() throws IOException
    {
        ...
        ByteBufferUtil.writeWithVIntLength(null == partitionKey ? EMPTY_BYTE_BUFFER : partitionKey, out);
        ByteBufferUtil.writeWithVIntLength(null == rowMark ? EMPTY_BYTE_BUFFER : rowMark.mark, out);
     ...
     *
     * There are two versions of legacy PagingState format - one used by 2.1- and one used by 3.0+.
    ...
    ByteBuffer legacySerialize(boolean withRemainingInPartition) throws IOException
    {
        ...
        ByteBufferUtil.writeWithShortLength(null == partitionKey ? EMPTY_BYTE_BUFFER : partitionKey, out);
        ByteBufferUtil.writeWithShortLength(null == rowMark ? EMPTY_BYTE_BUFFER : rowMark.mark, out);
    ...
    private static boolean isLegacySerialized(ByteBuffer bytes)
    {
        ...
        // V3 encoded by 2.1 - sans remainingInPartition
        if (index == limit)
            return true;
    ...
    private static PagingState legacyDeserialize(ByteBuffer bytes, int protocolVersion) throws IOException
    {
        ...
        /*
         * 2.1 implementation of V3 protocol did not write remainingInPartition, but C* 3.0+ does, so we need
