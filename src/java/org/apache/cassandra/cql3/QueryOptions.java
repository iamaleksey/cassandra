...
public abstract class QueryOptions
{
                                      ...
                                      long nowInMicros)
    {
        return new DefaultQueryOptions(consistency,
                                       values,
                                       skipMetadata,
                                       new SpecificOptions(pageSize, pagingState, serialConsistency, timestamp, keyspace, nowInMicros),
                                       version);
    }
    ...
    {
        long nowInMicros = getSpecificOptions().nowInMicros;
        return Long.MIN_VALUE == nowInMicros
             ? FBUtilities.nowInSeconds()
             : (int) TimeUnit.MICROSECONDS.toSeconds(nowInMicros);
    }

    @VisibleForTesting
    public long getNowInMicros()
    {
        return getSpecificOptions().nowInMicros;
    }
    ...
    static class SpecificOptions
    {
        ...
        private final long nowInMicros;
    ...
    private static class Codec implements CBCodec<QueryOptions>
    {
        ...
        public QueryOptions decode(ByteBuf body, ProtocolVersion version)
        {
            ...
            if (!flags.isEmpty())
            {
                ...
                long nowInMicros = flags.contains(Flag.NOW_IN_MICROSECONDS) ? body.readLong() : Long.MIN_VALUE;
                options = new SpecificOptions(pageSize, pagingState, serialConsistency, timestamp, keyspace, nowInMicros);
            }
        ...
        public void encode(QueryOptions options, ByteBuf dest, ProtocolVersion version)
        {
            ...
            if (version.isGreaterOrEqualTo(ProtocolVersion.V5))
            {
                ...
                if (flags.contains(Flag.NOW_IN_MICROSECONDS))
                    dest.writeLong(options.getSpecificOptions().nowInMicros);
            }
        ...
        private EnumSet<Flag> gatherFlags(QueryOptions options, ProtocolVersion version)
        {
            ...
            if (version.isGreaterOrEqualTo(ProtocolVersion.V5))
            {
                ...
                if (options.getSpecificOptions().nowInMicros != Long.MIN_VALUE)
                    flags.add(Flag.NOW_IN_MICROSECONDS);
            }
