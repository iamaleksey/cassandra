...
final class InternodeOutboundTable extends AbstractVirtualTable
{
    ...
    private void addRow(SimpleDataSet dataSet, InetAddressAndPort addressAndPort, OutboundConnections connections)
    {
        dataSet.row(addressAndPort.address, addressAndPort.port)
               .column(SENT_COUNT, sum(connections, OutboundConnection::getSent))
               .column(SENT_BYTES, sum(connections, OutboundConnection::getSentBytes))
               .column(PENDING_COUNT, sum(connections, OutboundConnection::getPending))
               .column(PENDING_BYTES, sum(connections, OutboundConnection::getPendingBytes))
               .column(EXPIRED_COUNT, sum(connections, OutboundConnection::droppedDueToTimeout))
               .column(EXPIRED_BYTES, sum(connections, OutboundConnection::droppedBytesDueToTimeout))
               .column(ERROR_COUNT, sum(connections, OutboundConnection::droppedDueToError))
               .column(ERROR_BYTES, sum(connections, OutboundConnection::droppedBytesDueToError))
               .column(OVERFLOW_COUNT, sum(connections, OutboundConnection::droppedDueToOverload))
               .column(OVERFLOW_BYTES, sum(connections, OutboundConnection::droppedBytesDueToOverload))
